...
#define CAMERA_BUFFER_SIZE (CAMERA_RES_X * CAMERA_RES_Y * 2) // RGB565 format
...
//      * the screen one-by-one. `put_px` is just an example. It needs to be implemented by you. */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	if (GPIO_Pin == LCD_INT_Pin)
	{
		if (XPT2046_TouchPressed())
		{
			...
			{
				lcdFillCircle(y, x, 2, COLOR_GREENYELLOW);
				// Sử dụng sprintf để tạo chuỗi message
				char message[50]; // Đảm bảo kích thước đủ lớn
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : CAM_RST_Pin LCD_NRST_Pin */
  GPIO_InitStruct.Pin = CAM_RST_Pin|LCD_NRST_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOD, &GPIO_InitStruct);
